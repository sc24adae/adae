...
class Cqsim_sim:
    ...
    def start_scan(self):
        ...
        while (i<wait_num):
            ...
            # only start the theta job normally, cori job can only be backfilled
            if temp_job["machine"] == 1:
                i += 1
                win_count += 1
                continue
            ...
            if (self.module['node'].is_available(temp_job['reqProc'])):
                ...
                temp_wait = self.module['job'].wait_list()
                backfill_job = self.module['job'].backfill_list()
                self.backfill(temp_wait, backfill_job)
                break
            i += 1
            win_count += 1
        return
    ...
    def backfill(self, temp_wait, backfill_job):
        #self.debug.debug("# "+self.myInfo+" -- backfill",5)
        temp_wait_info = []
        max_num = len(temp_wait)
        max_num_backfill = len(backfill_job)
        i = 0
        j = 0
        while (i < max_num):
            temp_job = self.module['job'].job_info(temp_wait[i])
        ...
        # backfill cori after backfill theta
        while (j < max_num_backfill):
            temp_job = self.module['job'].job_info(backfill_job[j])
            temp_wait_info.append({"index": backfill_job[j], "proc": temp_job['reqProc'], \
                                   "node": temp_job['reqProc'], "run": temp_job['run'], "score": temp_job['score']})
            j += 1
