...
class Job_trace:
    ...
    def reset_data(self):
        ...
        self.job_wait_list=[]
        # backfill list for cori jobs only,
        # these cori jobs can only be backfilled after backfilling of theta jobs
        self.job_backfill_list = []
        self.job_run_list=[]
        #self.job_done_list=[]
        self.num_delete_jobs = 0
        ...
        while (self.i<self.read_num or self.read_num<=0) and temp_n<self.read_input_freq:
            ...
            if (self.j>=self.anchor):
                ...
                tempInfo = {'id':int(temp_dataList[0]),\
                            ...
                            # 0 from theta, 1 from cori
                            'machine':int(temp_dataList[17]),\
                            'start':-1,\
                            'end':-1,\
                            'score':0,\
                            'state':0,\
    ...
    def backfill_list(self):
        return self.job_backfill_list
    ...
    def job_submit (self, job_index, job_score = 0, job_est_start = -1):
        ...
        # submit cori jobs into backfilling queue
        if self.jobTrace[job_index]['machine']==1:
            self.job_backfill_list.append(job_index)
        else:
    ...
    def job_start (self, job_index, time):
        ...
        # if this is a cori job...
        if self.jobTrace[job_index]['machine']==1:
            self.job_backfill_list.remove(job_index)
        else:
